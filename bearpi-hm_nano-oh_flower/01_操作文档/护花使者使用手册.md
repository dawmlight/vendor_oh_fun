...
- **`手机需提前接入2.4G的WiFi（一般为WiFi名字后缀不带5G字样的），且不能是开放的WiFi，也就是手机连接时需要输入密码的WiFi，开发者可通过另外一台手机开启热点的方式产生这样一个WiFi`**
...
&emsp;添加完设备后，**按一下开发板的复位按键**，等待30S后，下拉“我的设备”界面，刷新界面，此时设备变为在线状态。点击设备，打开设备详情界面，即可查看当前绿植的土壤湿度以及空气的温湿度，点击浇水按钮即可远程控制设备浇水。
...
https://gitee.com/openharmony-sig/vendor_oh_fun/tree/master/bearpi-hm_nano-oh_flower
    ...
    1.3    解压“openharmony-sig-vendor_oh_fun-master.zip”文件，进入“bearpi-hm_nano-oh_flower”文件夹，获得以下文件
...
2.	安装驱动
    ...
    2.1    通过TypeC数据线，把电脑与BearPi-HM Nano连接。
    
    2.2 	双击打开“02_USB驱动“文件夹下的”CH341SER.EXE“文件，并点击安装，驱动安装成功将如下图所示。

    ![](./figures/安装驱动.png)

    注：若提示“安装失败”，请点击“卸载”，再点击“安装”

    2.3 	打开电脑设备管理器，查看开发板的连接的端口，该端口用于后续烧录代码及打印调试日志使用。
 
    ![](./figures/查看端口.png)

3.	烧录程序

    3.1 	打开“04_烧录工具”文件下的“HiBurn.exe”工具，并点击Refresh，在COM中选择上一步看到的COM号。然后点击Setting，并选择 Com settings。
    
    ![](./figures/选择端口.png)

    3.2 	在Com settings中设置Baud为：921600，点击确定。

    ![](./figures/设置波特率.png)

    3.3 	点击 Hiburn工具中的Select file按钮，在弹出的文件框中，选择“03_固件”文件下的OpenHarmony_flower_V0.1 文件。

    ![](./figures/编译固件.png)

    3.4 	点击Auto burn复选框，然后点击Connect，此时Connect按钮变成Disconnect，等待下载。
 
    ![](./figures/连接开发板.png)

    3.5 	复位开发板RESET按键，开始下载程序。

    ![](./figures/复位开发板.png)
    ![](./figures/等待烧录.png)
    
    3.6 	直到出现Execution Successful字样，程序下载完成。

    3.7 	下载完后，点击Disconnect按钮，便于后面调测使用。
