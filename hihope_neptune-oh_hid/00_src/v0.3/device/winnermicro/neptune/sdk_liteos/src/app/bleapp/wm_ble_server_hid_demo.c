...
static uint8_t ble_svc_pnp_info_default[7] = {0x02,0x44,0x30,0xec,0x4e,0x17,0x00};
...
#define BLE_SVC_DIS_MODEL_NUMBER_READ_PERM      BLE_GATT_CHR_F_READ_ENC
#define BLE_SVC_DIS_SERIAL_NUMBER_READ_PERM     BLE_GATT_CHR_F_READ_ENC
#define BLE_SVC_DIS_HARDWARE_REVISION_READ_PERM BLE_GATT_CHR_F_READ_ENC
#define BLE_SVC_DIS_FIRMWARE_REVISION_READ_PERM BLE_GATT_CHR_F_READ_ENC
#define BLE_SVC_DIS_SOFTWARE_REVISION_READ_PERM BLE_GATT_CHR_F_READ_ENC
#define BLE_SVC_DIS_MANUFACTURER_NAME_READ_PERM BLE_GATT_CHR_F_READ_ENC
#define BLE_SVC_DIS_SYSTEM_ID_READ_PERM         BLE_GATT_CHR_F_READ_ENC
...
#define ADV_LAST_TIMEOUT_MS  120000

#define MY_NOTIFY_FLAGS (BLE_GATT_CHR_F_READ | BLE_GATT_CHR_F_NOTIFY | BLE_GATT_CHR_F_INDICATE)
#define HIGH_DUTY_CONN_INTERVAL      1
#define LOW_DUTY_CONN_INTERVAL       0
...
extern void tls_ble_server_hid_uart_set_led_status(uint8_t status);

int tls_ble_server_demo_hid_conn_param_update_high_duty_slave();
...
hid_set_notify(uint16_t attr_handle, uint8_t cur_notify, uint8_t cur_indicate)
{
    ...
    if (report_idx == -1) {
        ...
        TLS_BT_APPL_TRACE_DEBUG( "%s: service %s, attr_handle %d, notify %d, indicate %d\r\n",
    ...
    if (rep_idx != -1 && lock_hid_data() == 0) {
        ...
        if (rc == 0) {
            if (handle_num == HANDLE_HID_KB_OUT_REPORT) {
                ...
                tls_ble_server_hid_uart_set_led_status(Leds_buffer[0]);
                TLS_BT_APPL_TRACE_ERROR("KB LED:%02x\r\n", Leds_buffer[0]);
            }
        }
    } else {
        return 2;
                   ...
                   struct ble_gatt_access_ctxt *ctxt, void *arg)
{
    ...
    case BLE_SVC_DIS_CHR_UUID16_PNP_INFO:
        info = (char *)ble_svc_pnp_info_default;
        data_len = 7;
        break;
    default:
        rc = BLE_ATT_ERR_UNLIKELY;
...
#define HID_BT_SLEEP_MODE_SWITCHING 1
/**
...
hid_advertise(ble_addr_t *direct_addr)
{
    ...
    //fields.tx_pwr_lvl = 0;//BLE_HS_ADV_TX_PWR_LVL_AUTO;
    ...
    if(direct_addr == NULL)
    {
        rc = tls_nimble_gap_adv(WM_BLE_ADV_IND, ADV_LAST_TIMEOUT_MS);
    }else
    {
        rc = tls_nimble_gap_adv(WM_BLE_ADV_DIRECT_IND_HDC, 0);
    }
    ...
    {
        wm_hid_dev.advertising = true;
    }
}

static void wm_ble_server_hid_check_permission(void *arg)
{
    uint32_t active_ticks = (uint32_t)arg;
#if HID_BT_SLEEP_MODE_SWITCHING
    TLS_BT_APPL_TRACE_API("enable controller sleep mode...active_ticks=%d, curr_ticks=%d, hid_dev_ticks=%d\r\n", active_ticks, tls_os_get_time(),wm_hid_dev.conn_ticks);
    if(wm_hid_dev.conn_ticks == active_ticks)
    {
...
hid_gap_evt_cb(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_CONNECT:
        ...
        if (event->connect.status == 0) {
            ...
            wm_hid_dev.connected = true;
            wm_hid_dev.high_duty = true;
            wm_hid_dev.conn_ticks = tls_os_get_time();
            //hid_clean_vars(&desc);
            if(desc.peer_id_addr.type == 0)
            {
                tls_ble_server_hid_uart_set_led_status(0x00);
                update_hid_dev_state(BLE_GAP_EVENT_CONNECT, &desc.peer_id_addr);
            }
            wm_hid_dev.advertising = false;

            tls_bt_async_proc_func(wm_ble_server_hid_check_permission,(void*)wm_hid_dev.conn_ticks,5000);
        } else {
            /* Connection failed; resume advertising. */
            hid_advertise(NULL);
            wm_hid_dev.connected = false;
            wm_hid_dev.conn_ticks = 0;
        }
        return 0;

    case BLE_GAP_EVENT_DISCONNECT:
        TLS_BT_APPL_TRACE_API( "disconnect; reason=0x%02x \r\n", (event->disconnect.reason>=0x200 && event->disconnect.reason<0x300)?event->disconnect.reason-0x200:event->disconnect.reason);
        wm_hid_dev.connected = false;
        wm_hid_dev.conn_ticks = 0;
        wm_hid_dev.advertising = false;
        ...
        {
            TLS_BT_APPL_TRACE_WARNING("Continue to adv...\r\n");
    ...
    case BLE_GAP_EVENT_ADV_COMPLETE:
        ...
        {
            update_hid_dev_state(BLE_GAP_EVENT_DISCONNECT, NULL); //ADV timeout, clear device adding state;
        }
        return 0;
    ...
    case BLE_GAP_EVENT_SUBSCRIBE:
        TLS_BT_APPL_TRACE_DEBUG( "subscribe event; conn_handle=%d attr_handle=%04X "
    ...
    case BLE_GAP_EVENT_NOTIFY_TX:
        TLS_BT_APPL_TRACE_DEBUG( "notify event; status=%d conn_handle=%d attr_handle=%04X type=%s\r\n",
                    event->notify_tx.status,
                    event->notify_tx.conn_handle,
                    event->notify_tx.attr_handle,
                    event->notify_tx.indication?"indicate":"notify");
        wm_hid_dev.conn_ticks = tls_os_get_time();  //refresh conn_ticks to avoid low_duty check repeatedly
        return 0;
...
int tls_ble_server_demo_hid_advertise(uint8_t state , void *direct_addr)
{
    ...
    {
       tls_nimble_gap_adv(WM_BLE_ADV_STOP,0); 
    }
...
{
    return wm_hid_dev.connected;
}
uint8_t tls_ble_server_demo_hid_advertising()
{
    return wm_hid_dev.advertising;
}
...
int tls_ble_server_demo_hid_disconnect(bool continue_to_adv)
{
    int rc = 0;
     ...
     if(wm_hid_dev.connected)
     {
        ...
        rc = ble_gap_terminate(wm_hid_dev.conn_handle, BLE_ERR_REM_USER_CONN_TERM);
     }
...
int tls_ble_server_demo_hid_mouse_notify(uint8_t *mouse_array, int length)
{
    ...
    if(!wm_hid_dev.connected) return rc;
...
{
    uint32_t duty_flag = (uint32_t)arg;
    
	TLS_BT_APPL_TRACE_API("conn param update complete; conn_handle=%d status=%d flag=%s\n",
		       conn_handle, status, duty_flag?"high-duty":"low-duty");
    switch(duty_flag)
    {
        case HIGH_DUTY_CONN_INTERVAL:
            wm_hid_dev.high_duty = true;
            break;
        case LOW_DUTY_CONN_INTERVAL:
            wm_hid_dev.high_duty = false;
            break;
        default:
            break;
    }    
...
int tls_ble_server_demo_hid_conn_param_update_high_duty_slave()
{
    ...
    if(!wm_hid_dev.connected) return BLE_HS_EDISABLED;

    if(wm_hid_dev.high_duty) return BLE_HS_EALREADY;

	params.itvl_min = 16;
	params.itvl_max = 32;
	params.slave_latency = 0;
	params.timeout_multiplier = 300;

	rc = ble_l2cap_sig_update(wm_hid_dev.conn_handle, &params,hid_conn_param_update_cb, (void*)HIGH_DUTY_CONN_INTERVAL);

    return rc;
}

int tls_ble_server_demo_hid_conn_param_update_low_duty_slave()
{
	int rc = 0;
    uint32_t curr_ticks;
    uint32_t diff_ticks;
    ...
    curr_ticks = tls_os_get_time();

    if(curr_ticks > wm_hid_dev.conn_ticks)
    {
        diff_ticks = curr_ticks - wm_hid_dev.conn_ticks;
    }else
    {
        diff_ticks = curr_ticks + (0xFFFFFFFF - wm_hid_dev.conn_ticks);
    }

    if(diff_ticks < HZ*30) return BLE_HS_EDISABLED;
     
	//params.itvl_min = 160;
	//params.itvl_max = 200;
	params.itvl_min = 100;
	params.itvl_max = 120;       //caution, very low duty leads to connection broken	
	params.slave_latency = 0;
	params.timeout_multiplier = 600;

	rc = ble_l2cap_sig_update(wm_hid_dev.conn_handle, &params,hid_conn_param_update_cb, (void*)LOW_DUTY_CONN_INTERVAL);
...
int tls_ble_server_demo_hid_init()
{
    ...
    if(g_ble_demo_hid_inited == 0)
    {
		...
		{	
            ...
            /*step 3: load bakeup status and start advertisement if needed*/
            //hid_advertise(NULL); 
