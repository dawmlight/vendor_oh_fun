...
static uint8_t sfn[8] =        {0x04, 0x00, 0x26, 0x00, 0x00, 0x00, 0x00, 0x00}; //ctrl+alt++   adv mode;
static uint8_t sfn_delete[8] = {0x04, 0x00, 0x27, 0x00, 0x00, 0x00, 0x00, 0x00}; //ctrl+alt+-  delete the oldest bonding device
static uint8_t sfn1[8] =       {0x04, 0x00, 0x1e, 0x00, 0x00, 0x00, 0x00, 0x00}; //ctrl+alt+1
static uint8_t sfn2[8] =       {0x04, 0x00, 0x1f, 0x00, 0x00, 0x00, 0x00, 0x00}; //ctrl+alt+2
static uint8_t sfn3[8] =       {0x04, 0x00, 0x20, 0x00, 0x00, 0x00, 0x00, 0x00}; //ctrl+alt+3
static uint8_t sfn4[8] =       {0x04, 0x00, 0x21, 0x00, 0x00, 0x00, 0x00, 0x00}; //ctrl+alt+4
static uint8_t sfn5[8] =       {0x04, 0x00, 0x22, 0x00, 0x00, 0x00, 0x00, 0x00}; //ctrl+alt+5
...
int check_hotkey_pressed(uint8_t *key_array, int len)
{
    ...
    {
    #if 0
        TLS_BT_APPL_TRACE_DEBUG("### ctrl+shift++ detected\r\n");
        connected = tls_ble_server_demo_hid_connected();
        
        if(connected)
        {
            TLS_BT_APPL_TRACE_DEBUG("### hid service connected with remote devic, please disconect with us!!!\r\n");
            return 0;
        }else
        {
            TLS_BT_APPL_TRACE_DEBUG("### hid service start advertising\r\n");
    ...
    }else if(memcmp(key_array, sfn_delete, 4)==0)
    {
        ...
        TLS_BT_APPL_TRACE_DEBUG("### hid service delete oldest bonding device[%02x:%02x:%02x:%02x:%02x:%02x], rc=%d\r\n",
...
int process_hotkey(uint8_t key_action, uint8_t key_index)
{
    ...
    TLS_BT_APPL_TRACE_DEBUG("processing hotkey, action=%s, key_index=%d\r\n", key_action==1?"switching":"adding", key_index);
...
static void update_dev_to_nvram()
{
    int index = 0, i = 0, j = 0;
...
#if 0    
    TLS_BT_APPL_TRACE_DEBUG("Dump nvram information, then write to nvram, sizeof(ble_addr_t)=%d\r\n", sizeof(ble_addr_t));
    for(i = 0; i<45; i++)
    {
        j++;
 ...
 #endif
    btif_config_store_key_map(nvram_value, sizeof(nvram_value));
...
static void load_nvram_to_dev(uint8_t *nvram_info)
{
   ...
   uint8_t offset = 0;
    ...
    TLS_BT_APPL_TRACE_DEBUG("Dump nvram information, then load to dev array\r\n");
    for(i = 0; i<45; i++)
    {
        j++;
   ...
   if(index_action >= 0)
   {
        TLS_BT_APPL_TRACE_DEBUG("After loading nvram, adv started, and accept device (%d) only!!!\r\n", index_action);
...
int remove_hid_dev(ble_addr_t *addr)
{
    ...
    if(found)
    {
        TLS_BT_APPL_TRACE_DEBUG("Remove hid dev %d, it is the oldest one\r\n", index);
