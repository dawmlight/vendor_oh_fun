...
int ble_store_config_persist_our_secs()
{
    ...
    // ble_store_config_persist_flush();
    printf("Flush the cccd to the flash immediately\r\n");   // we may loss the event, so we need to flush it, anyway ,we will do a kvstore later   
    btif_config_flush(1);
...
void ble_store_config_conf_init()
{
    ...
    rc = ble_nvs_restore_sec_keys();
    if (rc == 0) {
        BLE_HS_LOG(INFO,"NVS operation failed, can't retrieve the bonding info\r\n");
    }
